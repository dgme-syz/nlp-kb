    ...
    for x in [train, val]:    
        with open(x, "r", encoding='utf-8') as f:
            for line in f:
                ...
                # 生成一个不重复的 8 位数字
                    entitys[e1] = len(entitys)
                if e2 not in entitys:
                    entitys[e2] = len(entitys)
...
def load_data(file):
    ...
    return entity2id, relation2id, train_triplets, valid_triplets

class RGCNDataset(Dataset):
    def __init__(self, triplets, batch_size, split_size, num_entity, num_rels, negative_rate):
        self.triplets = triplets
        self.data = generate_sampled_graph_and_labels(triplets, batch_size, split_size, num_entity, num_rels, negative_rate) 
...
""" 
    引用自 https://github.dev/JinheonBaek/RGCN/utils.py
"""
